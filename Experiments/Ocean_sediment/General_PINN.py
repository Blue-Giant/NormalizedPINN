...
class MscaleDNN(tn.Module):
    ...
    def loss_in(self, Xin=None, T_in=None, loss_type='l2_loss', scale2lncosh=0.5, ws=0.001,
                           ds=0.002, d_x = None, X_scale=1.0, T_scale=1.0, type2ds='linear', res2pde='modify'):
        '''
        Args:
        ...
        XT = torch.mul(Xin, self.mat2X) + torch.mul(T_in, self.mat2T)

        DS = d_x(Xin, ds)

        # 计算内部点损失 dx dxx
        UNN = self.DNN(XT, scale=self.factor2freq, sFourier=self.sFourier)
        grad2UNN = torch.autograd.grad(UNN, Xin, grad_outputs=torch.ones_like(Xin),
                                       create_graph=True, retain_graph=True)
        dUNN2x = grad2UNN[0]
        # dUNN2x = -tnf.relu(-grad2UNN[0])#这样不行的话就考虑用绝对值 我们希望gradUNN2x 是负的 所以把为正的弄为0
        dUNNxx = torch.autograd.grad(dUNN2x, Xin, grad_outputs=torch.ones_like(Xin),
        ...
        # Ds(X) = (10-X) * ds
        if type2ds == 'constant':
            res = dUNN2t + ws * dUNN2x - torch.mul(DS, dUNNxx)
        else:
            dD2dx = torch.autograd.grad(DS, Xin, grad_outputs=torch.ones_like(Xin),
                                        create_graph=True, retain_graph=True)[0]
            res = dUNN2t + ws * dUNN2x - (torch.mul(dUNN2x, dD2dx) + torch.mul(DS, dUNNxx))
    ...
    def loss_in2NormalizeX(self, Normalize_Xin=None, T_in=None, loss_type='l2_loss', scale2lncosh=0.5, ws=0.001,
        ...
        # loss
        # Ds(X) = (10-X) * ds
        if type2ds == 'constant' and res2pde != 'modify':
            res = dUNN2t + (1.0/X_scale) * ws * dUNN2x - (1.0/np.square(X_scale)) * torch.mul(DS, dUNNxx)
        elif type2ds == 'constant' and res2pde == 'modify':
            res = np.square(X_scale) * dUNN2t + X_scale * ws * dUNN2x - torch.mul(DS, dUNNxx)
        else:
            dD2dx = torch.autograd.grad(DS, Normalize_Xin, grad_outputs=torch.ones_like(Normalize_Xin),
                                        create_graph=True, retain_graph=True)[0]
            if res2pde != 'modify':
                res = dUNN2t + (1.0/X_scale) * ws * dUNN2x - \
                      (1.0/np.square(X_scale)) * (torch.mul(dUNN2x, dD2dx)+torch.mul(DS, dUNNxx))
            else:
                res = np.square(X_scale) * dUNN2t + X_scale * ws * dUNN2x - (torch.mul(dUNN2x, dD2dx) + torch.mul(DS, dUNNxx))
    ...
    def loss_in2NormalizeT(self, X_in=None, Normalize_Tin=None, loss_type='l2_loss', scale2lncosh=0.5, ws=0.001,
        ...
        if type2ds == 'constant' and res2pde != 'modify':
            ...
            dD2dx = torch.autograd.grad(DS, X_in, grad_outputs=torch.ones_like(X_in),
                                        create_graph=True, retain_graph=True)[0]
            if res2pde != 'modify':
                res = (1.0/T_scale) * dUNN2t + ws * dUNN2x - \
                      torch.add(torch.mul(dUNN2x, dD2dx), torch.mul(DS, dUNNxx))
            else:
                res = dUNN2t + T_scale * ws * dUNN2x - \
                      T_scale * torch.add(torch.mul(dUNN2x, dD2dx), torch.mul(DS, dUNNxx))
        if str.lower(loss_type) == 'l2_loss':
            loss_it = torch.mean(torch.square(res))
        elif str.lower(loss_type) == 'lncosh_loss':
            loss_it = (1 / scale2lncosh) * torch.mean(torch.log(torch.cosh(scale2lncosh * res)))
    ...
    def loss_in2NormalizeXT(self, Normalize_Xin=None, Normalize_Tin=None, loss_type='l2_loss', scale2lncosh=0.5,
        ...
        if type2ds == 'constant' and res2pde != 'modify':
            ...
            if res2pde != 'modify':
                ...
                res = np.square(X_scale) * dUNN2t + T_scale * X_scale * ws * dUNN2x - \
                      T_scale * (torch.mul(dUNN2x, dD2dx) + torch.mul(DS, dUNNxx))
...
def solve_Ocean_PDE(R):
    ...
    if R['testData_model'] == 'random_generate':
        # 生成测试数据，用于测试训练后的网络
        mesh_size = 101
        test_bach_size = mesh_size * mesh_size
        # ---------------------------------------------------------------------------------#
        x_coord = np.linspace(region_l, region_r, mesh_size).reshape(-1, 1)
        t_coord = np.linspace(init_time, end_time, mesh_size).reshape(-1, 1)
        meshX, meshT = np.meshgrid(x_coord, t_coord)
        x_points = np.reshape(meshX, newshape=[-1, 1])
        t_points = np.reshape(meshT, newshape=[-1, 1])
        test_xy_bach_no = np.concatenate([x_points, t_points], axis=-1)
        test_xy_bach = np.concatenate([x_points, t_points], axis=-1)
        ...
        x_test = x_data['y']
        t_test = t_data['x']
        shape2test = np.shape(x_test)
        mesh_size = shape2test[0]
        ...
        test_xy_bach[:, 1] = test_xy_bach[:, 1]

        z_test = z_data['z']
        z_test = z_test.reshape(-1, 1)

        if R['shuffle_data']:
            index2data = np.arange(start=0, stop=len(x_test), step=1)
            np.random.shuffle(index2data)
            z_test = z_test[index2data, :]
            test_xy_bach = test_xy_bach[index2data, :]

        saveData.save_testData_or_solus2mat(test_xy_bach, dataName='testXY', outPath=R['FolderName'])

        Utrue2test = torch.from_numpy(z_test)
        if True == R['use_gpu']:
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        t_in_batch = dataUtilizer2torch.rand_it(
            batchsize_it, 1, region_a=init_time, region_b=end_time, to_float=True, to_cuda=R['use_gpu'],
            gpu_no=R['gpuNo'], use_grad2x=True, lhs_sampling=True)

        t_bd_batch = dataUtilizer2torch.rand_it(
            batchsize_bd, 1, region_a=init_time, region_b=end_time, to_float=True, to_cuda=R['use_gpu'],
        ...
        UNN2train, loss_it = model.loss_in(
            Xin=x_in_batch, T_in=t_in_batch, loss_type=R['loss_type'], ws=Ws, ds=Ds, d_x=d_x, X_scale=X_scale,
            T_scale=T_scale, type2ds=R['ds'])
    ...
    plotData.plotTrain_loss_1act_func(loss_all, lossType='loss', outPath=R['FolderName'],
                                      yaxis_scale=True)
    plotData.plotTrain_loss_1act_func(loss_init_all, lossType='loss_init', outPath=R['FolderName'],
                                      yaxis_scale=True)
    ...
    if True == R['use_gpu']:
        ...
        utrue2test_numpy = utrue2test_numpy.reshape([mesh_size, mesh_size])

        unn2test_numpy = UNN2test.cpu().detach().numpy()
        unn2test_numpy = unn2test_numpy.reshape([mesh_size, mesh_size])
    else:
...
if __name__ == "__main__":
    ...
    if R['model2NN'] == 'Fourier_SubDNN':
        ...
        R['hidden_layers'] = (100, 80, 50, 50, 30)  # 1*100+100*80+80*50+50*50+50*30+30 = 16130
        # R['hidden_layers'] = (120, 80, 50, 50, 30)  # 1*120+120*80+80*50+50*50+50*30+30 = 16130
        # R['hidden_layers'] = (120, 80, 60, 60, 40)  # 1*120+120*80+80*60+60*60+60*40+40 = 20560
