...
class MscaleDNN(tn.Module):
    ...
    def loss_in2ocean(self, X_in=None, T_in=None, loss_type='l2_loss', scale2lncosh=0.5, ws=0.001, ds=0.002,
                      d_x = None, X_scale=None, type2ds='linear'):
        '''
        Args:
            X_in: 输入的内部深度（x or z)
            T_in: 输入的内部时间 (t)
        ...
        # loss
        # Ds(X) = (10-X) * ds
        res = np.square(X_scale) * dUNN2t + ws * dUNN2x - torch.mul(DS, dUNNxx)
...
def solve_Multiscale_PDE(R):
    ...
    if R['Norm'] == True:
        if R['Norm_type'] == 'X_only':
            X_scale = X_range
            T_scale = 1
        elif R['Norm_type'] == 'T_only':
            X_scale = 1
            T_scale = T_range
        elif R['Norm_type'] == 'Both':
            X_scale = X_range
            T_scale = T_range
    else:
        X_scale = 1
    ...
    if equation == 1:
        ...
        C0 = 1
        # A3的第一项
        end_time = T_range / T_scale
        region_r = X_range / X_scale
        Ds = ds * T_scale
        Ws = ws * T_scale * X_scale
    ...
    if R['testData_model'] == 'random_generate':
        ...
        test_xy_bach_no = np.concatenate([x_repeat * X_scale, t_repeat * T_scale], -1)
    ...
    t_train = np.linspace(init_time, end_time, 200, dtype=np.float32).reshape(-1, 1)
    ...
    if True == R['use_gpu']:
        ...
        t_train = t_train.cuda(device='cuda:' + str(R['gpuNo']))
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        if i_epoch < (R['max_epoch']*0.3):
            portion = 0.3
            split = 0.1
            # t_in_batch = dataUtilizer2torch.rand_it_zeromore(batchsize_it, 1, region_a=init_time,
            #                                             region_b=end_time, portion=portion, split=split,
            #                                             to_float=True, to_cuda=R['use_gpu'],
            #                                             gpu_no=R['gpuNo'], use_grad2x=True, lhs_sampling=True)
            # x_in_batch = dataUtilizer2torch.rand_it_zeromore(batchsize_it, input_dim, region_a=region_l,
            #                                                  region_b=region_r,
            #                                                  to_float=True, to_cuda=R['use_gpu'], portion=portion,
            #                                                  split=split,
            #                                                  gpu_no=R['gpuNo'], use_grad2x=True, lhs_sampling=True)
            ...
            #
            # x_init_batch = dataUtilizer2torch.rand_it(batchsize_init, 1, region_a=region_l+0.00000001, region_b=region_r,
            #                                           to_float=True, to_cuda=R['use_gpu'],
            #                                           gpu_no=R['gpuNo'], use_grad2x=False, lhs_sampling=True)
            t_bd_batch = dataUtilizer2torch.rand_it_zeromore(
                batchsize_bd, 1, region_a=init_time, portion=portion, split=split, region_b=end_time, to_float=True,
                to_cuda=R['use_gpu'], gpu_no=R['gpuNo'], use_grad2x=False, lhs_sampling=True)

            x_init_batch = dataUtilizer2torch.rand_it_zeromore(
                batchsize_init, 1, region_a=region_l+0.00000001, region_b=region_r, to_float=True, to_cuda=R['use_gpu'],
                portion=portion, split=split, gpu_no=R['gpuNo'], use_grad2x=False, lhs_sampling=True)
        else:
        ...
        UNN2train, loss_it = model.loss_in2ocean(X_in=x_in_batch, T_in=t_in_batch, loss_type=R['loss_type'],
                                                 ws=Ws, ds=Ds, d_x=d_x, X_scale=X_scale, type2ds=R['ds'])
