...
def solve_Ocean_PDE(R):
    ...
    if R['testData_model'] == 'random_generate':
        ...
        test_xy_bach_no = np.concatenate([x_points/X_scale, t_points], axis=-1)
        test_xy_bach = np.concatenate([x_points/X_scale, t_points], axis=-1)
        ...
        test_xy_bach[:, 0] = test_xy_bach[:, 0] / X_scale
    ...
    xl_bd_batch = np.ones(shape=[batchsize_bd, 1], dtype=np.float32) * 0.0
    xl_bd_batch = torch.from_numpy(xl_bd_batch)

    xr_bd_batch = np.ones(shape=[batchsize_bd, 1], dtype=np.float32) * 1.0
    ...
    if R['testData_model'] == 'random_generate':
        ...
        Utrue2test1 = u_true(torch.reshape(test_xy_torch[:, 0] * X_scale, shape=[-1, 1]),
                         torch.reshape(test_xy_torch[:, 1], shape=[-1, 1]))
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        x_in_batch = dataUtilizer2torch.rand_it(
            batchsize_it, input_dim, region_a=0.0, region_b=1.0, to_float=True, to_cuda=R['use_gpu'],
        ...
        x_init_batch = dataUtilizer2torch.rand_it(
            batchsize_init, 1, region_a=0.0, region_b=1.0, to_float=True, to_cuda=R['use_gpu'],
            gpu_no=R['gpuNo'], use_grad2x=False, lhs_sampling=True)
        # 计算损失函数
        ...
        UNN2train, loss_it = model.loss_in2NormalizeX(
            Normalize_Xin=x_in_batch, T_in=t_in_batch, loss_type=R['loss_type'], ws=Ws, ds=Ds, d_x=d_x, X_scale=X_scale,
