...
def solve_Ocean_PDE(R):
    ...
    if equation == 1:
        ...
        u_left = lambda x, t: torch.ones_like(x) * C0
    ...
    if R['testData_model'] == 'random_generate':
        ...
        test_xy_bach_no = np.concatenate([x_points * X_scale, t_points * T_scale], -1)
        test_xy_bach = np.concatenate([x_points / X_scale, t_points / T_scale], -1)
    ...
    t_init_batch = np.ones(shape=[batchsize_init, 1], dtype=np.float32) * 0.0
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        UNN2train, loss_it = model.loss_in2NormalizeXT(
            Normalize_Xin=x_in_batch, Normalize_Tin=t_in_batch, loss_type=R['loss_type'], ws=Ws, ds=Ds, d_x=d_x,
