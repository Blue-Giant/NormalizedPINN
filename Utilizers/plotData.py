...
def plotTrain_MSE_REL_1act_func(data2mse, data2rel, actName=None, outPath=None, xaxis_scale=False,
    ...
    if str.lower(actName) == 'srelu':
        fntmp = '%s/trainErr_%s' % (outPath, 'sReLU')
    elif str.lower(actName) == 'sin':
        fntmp = '%s/trainErr_%s' % (outPath, 'sin')
    elif str.lower(actName) == 's2relu':
        fntmp = '%s/trainErr_%s' % (outPath, 's2ReLU')
    elif str.lower(actName) == 's3relu':
        fntmp = '%s/trainErr_%s' % (outPath, 's3ReLU')
    elif str.lower(actName) == 'csrelu':
        fntmp = '%s/trainErr_%s' % (outPath, 'CsReLU')
    elif str.lower(actName) == 'relu':
        fntmp = '%s/trainErr_%s' % (outPath, 'ReLU')
    elif str.lower(actName) == 'elu':
        fntmp = '%s/trainErr_%s' % (outPath, 'elu')
    elif str.lower(actName) == 'tanh':
        fntmp = '%s/trainErr_%s' % (outPath, 'tanh')
    elif str.lower(actName) == 'sintanh':
        fntmp = '%s/trainErr_%s' % (outPath, 'sintanh')
    elif str.lower(actName) == 'singauss':
        fntmp = '%s/trainErr_%s' % (outPath, 'singauss')
    elif str.lower(actName) == 'gauss':
        fntmp = '%s/trainErr_%s' % (outPath, 'gauss')
    elif str.lower(actName) == 'mexican':
        fntmp = '%s/trainErr_%s' % (outPath, 'mexican')
    elif str.lower(actName) == 'modify_mexican':
        fntmp = '%s/%s' % (outPath, 'Ummexican2test')
    elif str.lower(actName) == 'sin_modify_mexican':
        fntmp = '%s/%s' % (outPath, 'Usm_mexican2test')
    # DNN_tools.mySaveFig(plt, fntmp, ax=ax, isax=1, iseps=0)
...
def plot_2solutions2test(exact_solu2test, predict_solu2test,  coord_points2test=None,
    ...
    if subfig_type == 1:
        ...
        xy = (0.64, 0.265)
        xy2 = (0.36, 0.265)
        con = ConnectionPatch(xyA=xy2, xyB=xy, coordsA="data2old", coordsB="data2old", axesA=p2, axesB=p1)
        p2.add_artist(con)

        xy = (0.64, 0.21)
        xy2 = (0.36, 0.205)
        con = ConnectionPatch(xyA=xy2, xyB=xy, coordsA="data2old", coordsB="data2old",
                              axesA=p2, axesB=p1)
        p2.add_artist(con)
