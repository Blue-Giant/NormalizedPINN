...
def dictionary_out2file(R_dic, log_fileout):
    ...
    DNN_tools.log_string('Normalized the variable of PDE or not: %s\n' % (R_dic['Norm']), log_fileout)
    DNN_tools.log_string('The variables that have Normalized: %s\n' % (R_dic['Norm_type']), log_fileout)
    ...
    DNN_tools.log_string('Batch-size 2 initial for training: %s\n' % str(R_dic['batch_size2init']), log_fileout)
    DNN_tools.log_string('Initial boundary penalty: %s\n' % str(R_dic['init_boundary_penalty']), log_fileout)
    DNN_tools.log_string('Initial penalty for initial condition: %s\n' % str(R_dic['init_penalty2init']), log_fileout)
...
def print_and_log_train_Case2(i_epoch, run_time, tmp_lr, temp_penalty_bd, temp_penalty_init, pwb,
                                        ...
                                        log_out=None, max_epoch = 50000):
    # 将运行结果打印出来
    print('train epoch: %d / %d, time: %.3f' % (i_epoch, max_epoch, run_time))
